...
const NAV_ITEMS = [
    ...
    { id: 'queue', label: 'Fila e guichês', icon: LiaClipboardListSolid },
    ...
    return (
        <div className="dashboard-layout">
            ...
            <main className="dashboard-content procon-admin-page">
                ...
                {active === 'queue' && <section className="data-card procon-module-card"><div className="card-header"><div><h2>Fila e guichês</h2><p>Chamadas alternadas entre atendimentos prioritários e normais, com controle de guichês.</p></div><button className="procon-card-action" onClick={() => setShowQueue(true)}>Gerenciar chamadas e guichês</button></div><div className="procon-kpis compact"><article><span>Aguardando</span><strong>{waiting}</strong></article><article><span>Chamando</span><strong>{queue.filter(item => item.status === 'Chamando').length}</strong></article><article><span>Em atendimento</span><strong>{queue.filter(item => item.status === 'Em Atendimento').length}</strong></article><article><span>Concluídos hoje</span><strong>{queue.filter(item => item.status === 'Concluído').length}</strong></article></div><div className="procon-record-list">{queue.filter(item => item.status === 'Aguardando').map(item => <article key={item.id}><div><strong>{item.senha}</strong><span>{item.nome}</span></div><small>{item.assunto}</small><b>{item.prioridade ? 'Prioritário' : 'Normal'}</b></article>)}</div></section>}
