...
export default function AdministrativeReports() {
    ...
    const activeReports = useMemo(() => Object.entries(REPORTS).filter(([id]) => canAccessModule(settings, role, currentUser?.email, id, 'admin')), [currentUser?.email, role, settings]);
    useEffect(() => {
