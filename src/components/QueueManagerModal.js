...
const QueueManagerModal = ({ onClose, lockedService = '' }) => {
    ...
    const writeCalendarRecord = (writer, ticketRefId, ticket, now) => {
        ...
        writer.set(doc(firestore, 'atendimento-calendario', `${ticket.sessaoGuicheId}_${ticketRefId}`), {
            ...
            horarioFim: now,
            entradaFilaEm: ticket.entradaFilaEm || ticket.chegadaRecepcaoEm || ticket.ordemFilaEm || ticket.criadoEm || null,
            chamadoEm: ticket.chamadoEm || null,
            atendimentoIniciadoEm: ticket.atendimentoIniciadoEm || ticket.chamadoEm || null,
            concluidoEm: now,
